        ...
        if [ "$1" == "--all" ]; then
               echo -e "\nrunning all extended tests\n"
                for dir in $(echo test/TEST-*); do
                       ...
                       [[ "$TESTRES" =~ "PASS" ]] && rm -rf /var/tmp/$TESTDIR &>/dev/null
                       cd ${TEST_BASE_DIR%%/test}
                done
	else
                if [ -n "$1" && -z "$2" ]; then
                       echo -e "\nrunning extended test $1\n"
                       dir="test/$1"
                       cd "$dir"
                       sed -i '/SKIP_INITRD=yes/d' test.sh
                       testname=$(basename $dir)
                       ...
                       echo -e "\npreparing next extended test\n"
                       ./test.sh $2 2>&1>> ${TEST_BASE_DIR%%/test}/logs/$testname-setup.log
                fi
        fi
fi
